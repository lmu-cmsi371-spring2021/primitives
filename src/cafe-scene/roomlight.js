...
export default (() => {
  ...
  const drawLight = (renderingContext, options) => {
    const brightness = +(options.brightness || DEFAULT_BRIGHTNESS)

    const radialGradient = renderingContext.createRadialGradient(0, SHADE_RADIUS, 1, 0, SHADE_RADIUS, LIGHT_RADIUS)
  ...
  const drawShade = renderingContext => {
    renderingContext.fillStyle = BASE_COLOR
    renderingContext.strokeStyle = BASE_COLOR

    renderingContext.fillRect(-(SHADE_HOLD_WIDTH / 2), -(SHADE_HOLD_HEIGHT / 2), SHADE_HOLD_WIDTH, SHADE_HOLD_HEIGHT)
  ...
  const drawRoomLight = (renderingContext, options) => {
