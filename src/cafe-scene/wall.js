...
export default (() => {
  ...
  const drawBase = (renderingContext, options) => {
    ...
    if (options.hasStripes) {
      ...
      for (let i = 0; i < STRIPE_COUNT; i += 2) {
        renderingContext.fillRect(-(WALL_WIDTH / 2) + STRIPE_WIDTH * i, -(WALL_HEIGHT / 2), STRIPE_WIDTH, WALL_HEIGHT)
      }
    }
  }

  const drawAccent = renderingContext => {
    ...
    for (let i = 0; i < WALL_WIDTH / ACCENT_PANEL_WIDTH / 2 - 1; i += 1) {
      renderingContext.strokeRect(
        ...
        WALL_HEIGHT / 2 - ACCENT_HEIGHT,
        ACCENT_PANEL_WIDTH,
        ACCENT_HEIGHT
      )
    }

    renderingContext.fillRect(-(WALL_WIDTH / 2), WALL_HEIGHT / 2 - ACCENT_HEIGHT, WALL_WIDTH, ACCENT_PANEL_WIDTH / 2)

    renderingContext.strokeRect(-(WALL_WIDTH / 2), WALL_HEIGHT / 2 - ACCENT_HEIGHT, WALL_WIDTH, ACCENT_PANEL_WIDTH / 2)
  }

  const drawWall = (renderingContext, options) => {
