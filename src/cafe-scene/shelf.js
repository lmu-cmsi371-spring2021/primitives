...
export default (() => {
  ...
  const drawBase = renderingContext => {
    ...
    renderingContext.fillRect(
      -(SHELF_WIDTH / 2) + BOARD_WIDTH,
      -(SHELF_HEIGHT / 2) + BOARD_WIDTH,
      SHELF_WIDTH - BOARD_WIDTH * 2,
      SHELF_HEIGHT - CABINET_HEIGHT
    )

    renderingContext.fillStyle = MAIN_COLOR
    for (let i = 0; i < 4; i += 1) {
      renderingContext.fillRect(-(SHELF_WIDTH / 2), -(SHELF_HEIGHT / 2) + LEVEL_HEIGHT * i, SHELF_WIDTH, BOARD_WIDTH)
    }
    ...
    renderingContext.strokeRect(
      -(SHELF_WIDTH / 2) + BOARD_WIDTH,
      SHELF_HEIGHT / 2 - CABINET_HEIGHT + BOARD_WIDTH * 2,
      SHELF_WIDTH - BOARD_WIDTH * 2,
      LEVEL_HEIGHT
    )
  ...
  const drawShelf = renderingContext => {
