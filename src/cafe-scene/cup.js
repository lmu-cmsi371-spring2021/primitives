...
export default (() => {
  ...
  const drawBase = renderingContext => {
  ...
  const drawHandle = renderingContext => {
    renderingContext.save()
    renderingContext.beginPath()
    renderingContext.moveTo(0, 0)
    renderingContext.bezierCurveTo(
      HANDLE_CURVE.ctrlPt1.x,
      HANDLE_CURVE.ctrlPt1.y,
      HANDLE_CURVE.ctrlPt2.x,
      HANDLE_CURVE.ctrlPt2.y,
      HANDLE_CURVE.endPt.x,
      HANDLE_CURVE.endPt.y
    )
  ...
  const drawSteamCurve = (renderingContext, xOffset, yOffset) => {
    renderingContext.beginPath()
    renderingContext.moveTo(xOffset, STEAM_Y_OFFSET + yOffset)
    renderingContext.quadraticCurveTo(
      STEAM_CURVE.ctrlPt1.x + xOffset,
      STEAM_CURVE.ctrlPt1.y + yOffset,
      xOffset,
      STEAM_Y_OFFSET - STEAM_HEIGHT + yOffset
    )
    renderingContext.quadraticCurveTo(
      STEAM_CURVE.ctrlPt2.x + xOffset,
      STEAM_CURVE.ctrlPt2.y + yOffset,
      xOffset,
      STEAM_Y_OFFSET - STEAM_HEIGHT * 2 + yOffset
    )
    renderingContext.stroke()
    renderingContext.closePath()
  }

  const drawSteamSet = (renderingContext, options) => {
    const opacity = options.steamOpacity || DEFAULT_STEAM_OPACITY
    renderingContext.save()
  ...
  const drawCup = (renderingContext, options) => {
    renderingContext.save()
